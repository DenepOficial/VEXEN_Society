...
from app.society.associates import (
    SocietyCog,
)
...
class VexenSocietyBot(commands.Bot):
    ...
    async def setup_hook(
        ...
        log.info(
            "%d botones persistentes de comunidad restaurados.",
            restored_views,
        )
        ...
        if not self.settings.sync_commands:
            log.info(
                "SYNC_COMMANDS desactivado."
            )
            return

        if self.settings.guild_id is None:
            log.warning(
                "GUILD_ID no configurado; "
                "no se sincronizarán comandos."
            )
            return

        guild = discord.Object(
            id=self.settings.guild_id
        )
        ...
        if self.settings.guild_id is not None:
            for guild in list(self.guilds):
                if (
                    ...
                    log.warning(
                        "Abandonando guild no autorizado: "
                        "%s (%s)",
                        guild.name,
                        guild.id,
                    )
